...
public class Course {
    ...
    public boolean enrollStudent(final Student student) {
        String enrollStudentSql = String.format("INSERT INTO %s (student_id, course_id) VALUES (%d, %d)", STUDENT_COURSE_TABLE_NAME, student.id(), id);

        try {
            if (QueryExecutor.createAndObtainId(enrollStudentSql) != -1) {
                return true;
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public List<Student> studentList() {
    	String findStudentListSql = String.format("SELECT * FROM student s JOIN %s sc ON sc.student_id = s.id WHERE sc.course_id = %d", STUDENT_COURSE_TABLE_NAME, id);
    	
    	List<Student> resultList = new LinkedList<>();

        try {
            ResultSet student = QueryExecutor.read(findStudentListSql);

            while(student.next()) {
                resultList.add(new Student(student.getInt("id"), student.getString("first_name"),
                        student.getString("last_name"), student.getInt("index_number")));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }

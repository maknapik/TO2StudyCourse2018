...
public class AccountOverviewController {
	...
	private void initialize() {
		...
		editButton.disableProperty().bind(
		        Bindings.createBooleanBinding(() -> transactionsTable.getSelectionModel().getSelectedItems().size() != 1, transactionsTable.getSelectionModel().getSelectedItems()));
	}
    ...
    public void handleAddAction(ActionEvent event) {
	    ...
	    if(appController.showTransactionEditDialog(transaction)) {
            data.getTransactions().addAll(transaction);
        }
    }
